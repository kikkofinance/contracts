...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await KikkoReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.kikko.address, '100', '1000', { from: owner });

        await this.kikko.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setKikkoReferral(this.referral.address, { from: owner });
